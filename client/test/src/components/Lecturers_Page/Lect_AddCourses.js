...
const LectAddCourses = (props)=> {
 ...
 function handleSubmit(e){
    ...
    Axios.post("https://vercel-backend-test-azure.vercel.app/courses",{
      ...
      lect_id: lect_id  //gotten from ./lectHome
    }).then((result)=>{
      console.log(result);
   ...
   return(
    ...
    <div className={`sidebar ${isOpen ? 'open' : 'close'}`} ref={sidebarRef}>
      ...
      <ul>
        ...
        <li><button className="sidebar-links" onClick={()=>{navigate('/lectCourses', {state:{ lect_username, lect_id }})}}>Course List</button></li>  {/*Passes the username and matric_num into ./courses as state in the navigate function */}
